...
router.post('/getcsv', function(req, res){
  ...
  var data = [clientId]; // create an array from the mongo object so we can use .unshift() later
  // var headers = Object.keys(mongoObject);
  var headers = Object.keys(clientId); // create a header row from the object's keys/properties
	Client.find({_id: clientId}, function (err, client) {
		if (err) {
			...
			res.status(500).send(err);
				...
				data.unshift(headers); 	// push keys array to the beginning of data array
